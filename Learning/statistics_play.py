...
def statistics_play(dir_name):
    ...
    versions_sort.sort()
    versions = [version for _, version in versions_sort]
    ...
    for idx_first, first_player in enumerate(players):
        name_first = versions[idx_first]
        for idx_second, second_player in players:
            if idx_first == idx_second:
                continue
            name_second = versions[idx_first + idx_second + 1]
            game = play(first_player, second_player)
            if game.is_tie():
                stats["Tie"][name_first] += 1
                stats["Tie"][name_second] += 1
            elif game.get_winner() == PlayerRole.CROSSES:
                stats["Wins"][name_first] += 1
            else:
                stats["Wins"][name_second] += 1
